...
func (r Runner) runOnce(ctx context.Context, cfg Config, index int, iteration int, warmup bool, scheduledAt time.Time) Sample {
	...
	lifecycle := r.sampleLifecycle()
	sample := lifecycle.runPrepared(ctx, cfg, index, iteration, warmup, scheduledAt, prepared, preparedNS)
	if cleanup := lifecycle.cleanupAfterSample(ctx, cfg, sample); cleanup != nil {
...
func (l sampleLifecycle) runPrepared(ctx context.Context, cfg Config, index int, iteration int, warmup bool, scheduledAt time.Time, prepared PreparedRequest, preparedNS int64) Sample {
	batchSize := 1
	if cfg.Scenario == ScenarioBatch {
		batchSize = cfg.BatchSize
	}

	prestartedConfirmation, cancelPrestartedConfirmation := l.startParallelConfirmation(ctx, cfg, prepared)
	if cancelPrestartedConfirmation != nil {
		defer cancelPrestartedConfirmation()
	}

	submission, err := l.execute(ctx, prepared)
	...
	if err == nil && classification.OK() && cfg.MeasurementMode == MeasurementModeWSConfirmation {
		...
		} else if prestartedConfirmation != nil {
			confirmed := <-prestartedConfirmation
			measured = confirmed.result
			measurementErr = confirmed.err
			if confirmed.err != nil {
				classification = lifecycle.Classification{Status: lifecycle.StatusTransportError, Reason: confirmed.err.Error()}
			}
		} else {
...
func (l sampleLifecycle) startParallelConfirmation(ctx context.Context, cfg Config, prepared PreparedRequest) (<-chan confirmationOutcome, context.CancelFunc) {
	if cfg.MeasurementMode != MeasurementModeWSConfirmation || prepared.Confirm == nil || prepared.Confirm.Wait == nil || len(prepared.ParallelRequests) == 0 {
		return nil, nil
	}
	confirmCtx, cancel := context.WithTimeout(ctx, cfg.ConfirmationTimeout)
	startedAt := time.Now().UTC()
	submission := netlatency.Result{
		Trace: netlatency.Trace{
			StartedAt: startedAt,
			Transport: transportName(prepared.Transport, "http"),
		},
	}
	ch := make(chan confirmationOutcome, 1)
	go func() {
		result, err := prepared.Confirm.Wait(confirmCtx, submission)
		ch <- confirmationOutcome{result: result, err: err}
	}()
	return ch, cancel
}

func (l sampleLifecycle) cleanupAfterSample(ctx context.Context, cfg Config, sample Sample) *CleanupResult {
	if l.cleanup == nil || !cfg.Cleanup.Enabled || cfg.Cleanup.Scope != CleanupScopeAfterSample {
		return nil
	}
	if !shouldCleanupAfterSample(sample) {
		return nil
	}
	cleanup := l.cleanup.AfterSample(ctx, sample)
	return &cleanup
}

func shouldCleanupAfterSample(sample Sample) bool {
	return sample.OK || len(sample.OrderRefs) > 0
}

func (l sampleLifecycle) execute(ctx context.Context, prepared PreparedRequest) (netlatency.Result, error) {
	if prepared.Execute != nil {
		return prepared.Execute(ctx)
	}
	if len(prepared.ParallelRequests) > 0 {
		return l.client.DoParallelFastest(ctx, prepared.ParallelRequests)
	}
	return l.client.Do(ctx, prepared.Request)
}
...
func writeDelayNS(scheduledAt time.Time, trace netlatency.Trace) int64 {
	if scheduledAt.IsZero() || trace.StartedAt.IsZero() {
		return 0
	}
	writeAt := trace.StartedAt
	if trace.WroteRequestAtNS > 0 {
		writeAt = trace.StartedAt.Add(time.Duration(trace.WroteRequestAtNS))
	}
	if writeAt.Before(scheduledAt) {
		return 0
	}
	return writeAt.Sub(scheduledAt).Nanoseconds()
}
