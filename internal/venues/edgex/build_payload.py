...
def build(
    ...
    client_id = client_order_id(params, req, 0)
    ...
    return {
        ...
        "metadata": {
            ...
            "order_type": benchmark_order_type(enum_wire(order_type), str(body.get("timeInForce") or ""), bool(params.get("reduce_only", False))),
            "time_in_force": str(body.get("timeInForce") or ""),
            "speed_bump_ns": 0,
            "speed_bump_ms": 0,
            "speed_bump_source": "edgeX official API docs do not document a fixed order-entry speed bump",
            "cleanup_orders": [
                {
                    "venue": "edgex",
                    "contract_id": str(params["contract_id"]),
            ...
            "confirmation": confirmation_metadata(params, account_id, stark_private_key, async_client, body["clientOrderId"], enum_wire(order_type), str(body.get("timeInForce") or "")),
        },
    }
...
def client_order_id(params: dict[str, Any], req: dict[str, Any], offset: int) -> str:
    if params.get("client_order_id"):
        return str(params["client_order_id"]) if offset == 0 else str(int(params["client_order_id"]) + offset)
    ...
    run_id = params.get("run_id")
    if run_id:
        seed = f"{run_id}:{req.get('iteration', 0)}:{params.get('contract_id')}:{params.get('side', 'BUY')}:{offset}".encode()
        return str(int.from_bytes(hashlib.blake2b(seed, digest_size=8).digest(), "big") % 9_000_000_000_000_000_000)
...
def benchmark_order_type(order_type: str, time_in_force: str, reduce_only: bool) -> str:
    if reduce_only:
        return "reduce_only"
    ...
    if normalized_type == "market":
        return "market"
    if normalized_tif == "post_only":
        return "post_only"
    if normalized_tif in ("immediate_or_cancel", "fill_or_kill"):
        return normalized_tif
    return normalized_type or "limit"
...
def confirmation_metadata(
    ...
    ws_url = private_ws_url(str(params.get("ws_url") or params.get("base_url", DEFAULT_BASE_URL)), account_id)
    timestamp = int(time.time() * 1000)
    sign_path = f"/api/v1/private/wsaccountId={account_id}"
    sign_content = f"{timestamp}GET{sign_path}"
    from Crypto.Hash import keccak

    content_hash = keccak.new(digest_bits=256)
    content_hash.update(sign_content.encode())
    signature = async_client.sign(content_hash.digest())
    return {
        "venue": "edgex",
        "ws_url": ws_url,
        "headers": {
            "X-edgeX-Api-Timestamp": str(timestamp),
            "X-edgeX-Api-Signature": f"{signature.r}{signature.s}",
        },
        "client_order_ids": [client_id],
        "order_type": benchmark_order_type(order_type, time_in_force, bool(params.get("reduce_only", False))),
    }


def private_ws_url(base_url: str, account_id: int) -> str:
    if base_url.startswith("wss://") or base_url.startswith("ws://"):
        base = base_url.rstrip("/")
    elif base_url.startswith("https://"):
        base = "wss://" + base_url[len("https://"):].rstrip("/")
    elif base_url.startswith("http://"):
        base = "ws://" + base_url[len("http://"):].rstrip("/")
    else:
        base = "wss://" + base_url.rstrip("/")
    if base.endswith("/api/v1/private/ws"):
        return f"{base}?accountId={account_id}"
    return f"{base}/api/v1/private/ws?accountId={account_id}"
