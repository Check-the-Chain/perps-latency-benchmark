...
def build(
    ...
    if orders is None:
        ...
        orders = [order_from_params(params, req, offset, Cloid, price_for_offset(params, price, offset)) for offset in range(batch_size if scenario == "batch" else 1)]
    if scenario != "batch":
        orders = orders[:1]
    ...
    built: dict[str, Any] = {
        ...
        "body": compact_json(payload),
            "metadata": {
                ...
                "speed_bump_ns": 0,
                "speed_bump_ms": 0,
                "speed_bump_source": "hyperliquid priority semantics: cancels and post-only orders are prioritized before GTC/IOC; no documented fixed millisecond speed bump",
...
def price_for_offset(params: dict[str, Any], base_price: Decimal | None, offset: int) -> Decimal | None:
    price = base_price or Decimal(str(params["price"]))
    if offset <= 0:
        return price
    ...
    if step == 0:
        return price
    is_buy = str(params.get("side", "buy")).lower() == "buy"
    stepped = price - (step * offset) if is_buy else price + (step * offset)
    return valid_price(stepped, is_buy)
...
def order_cloid(params: dict[str, Any], req: dict[str, Any], offset: int, Cloid: Any) -> Any:
    ...
    if run_id and is_fill_likely(params):
        seed = f"{run_id}:{req.get('iteration', 0)}:{params.get('symbol', 'BTC')}:{params.get('side', 'buy')}:{offset}:{time.time_ns()}".encode()
...
def is_fill_likely(params: dict[str, Any]) -> bool:
    order_type = str(params.get("order_type", "")).lower()
    tif = str(params.get("tif", params.get("time_in_force", ""))).lower()
    return order_type in ("market", "ioc") or tif == "ioc"
