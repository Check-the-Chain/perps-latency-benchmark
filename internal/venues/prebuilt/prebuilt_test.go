...
func TestPrebuiltVenueUsesParallelRequests(t *testing.T) {
	firstBody := `{"first":true}`
	secondBody := `{"second":true}`
	venue, err := New(Config{
		Name: "test",
		URL:  "https://example.com/single",
		Builder: staticBuilder{built: payload.Built{
			ParallelRequests: []payload.BuiltHTTPRequest{
				{Body: &firstBody, Headers: map[string]string{"X-Order": "1"}},
				{URL: "https://example.com/other", Body: &secondBody},
			},
		}},
	})
	if err != nil {
	...
	if len(prepared.ParallelRequests) != 2 {
		t.Fatalf("parallel requests = %d", len(prepared.ParallelRequests))
	}
	if string(prepared.ParallelRequests[0].Body) != firstBody {
		t.Fatalf("first body = %s", prepared.ParallelRequests[0].Body)
	}
	if prepared.ParallelRequests[0].Header.Get("X-Order") != "1" {
		t.Fatalf("first header = %s", prepared.ParallelRequests[0].Header.Get("X-Order"))
	}
	if prepared.ParallelRequests[1].URL != "https://example.com/other" {
		t.Fatalf("second url = %s", prepared.ParallelRequests[1].URL)
	}
}
