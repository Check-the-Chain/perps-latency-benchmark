...
function LogoMark({ venue }: { venue: string }) {
  switch (venue) {
    ...
    case "extended":
      return <ExtendedLogo />
    case "grvt":
      return <LetterLogo label="G" tone="oklch(0.45 0.08 250)" />
    case "hyperliquid":
    ...
    case "variational_omni":
      return <LetterLogo label="VO" tone="oklch(0.62 0.15 75)" />
    default:
...
function EdgeXLogo() {
  return (
    <svg viewBox="0 0 24 24" className="size-full" aria-hidden>
      <rect width="24" height="24" fill="#101820" />
      <path d="M5 5h14v3H8.6v2.5H18v3H8.6V16H19v3H5V5Z" fill="#E8F0FF" />
      <path d="M6 19 19 5h-4L2 19h4Z" fill="#42D6A4" opacity=".9" />
    </svg>
  )
}
...
function HyperliquidLogo() {
  return (
    <svg viewBox="0 0 65.3 57" className="h-[82%] w-full" aria-hidden>
...
function PacificaLogo() {
  return (
    <svg viewBox="0 0 2000 2000" className="size-full" aria-hidden>
      <rect width="2000" height="2000" fill="#0E1724" />
